...
class ndnLcd : public ForwardingStrategy
{
public:
...
NS_OBJECT_ENSURE_REGISTERED (ndnLcd);
//-------------------------------------------------
TypeId
ndnLcd::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::ndnLcd")
    .SetParent<ForwardingStrategy> ()
    .SetGroupName ("Ndn")
    .AddConstructor<ndnLcd> ()
    .AddAttribute ("ID",
                   "ID",
                   UintegerValue (0),
                   MakeUintegerAccessor (&ndnLcd::consumerID),
                   MakeUintegerChecker<uint32_t> ())
    ;
  return tid;
}

ndnLcd::ndnLcd():dataNum(0), interestNum(0)
{	
...
void
ndnLcd::DoOnInterest (Ptr<Face> inFace,
                                Ptr<Interest> interest)
{
  CacheTag cacheTag;
...
void
ndnLcd::SatisfyPendingInterest (Ptr<Face> inFace,
                                            Ptr<const Data> data,
                                            Ptr<pit::Entry> pitEntry)
{
  if (inFace != 0)
...
void
ndnLcd::OnInterest (Ptr<Face> inFace,
                                Ptr<Interest> interest)
{
...
void
ndnLcd::DidReceiveUnsolicitedData (Ptr<Face> inFace,
                                               Ptr<const Data> data,
                                               bool didCreateCacheEntry)
{
  Ptr<Node> node = inFace->GetNode();
...
void
ndnLcd::DidReceiveSolicitedData (Ptr<Face> inFace,
                                               Ptr<const Data> data,
                                               bool didCreateCacheEntry)
{
  Ptr<Node> node = inFace->GetNode();
...
void
ndnLcd::OnData (Ptr<Face> inFace, Ptr<Data> data)
{	
  CacheTag cacheTag;
  ...
  while (pitEntry != 0)
    {
      ...
      ProducerTag proTag;
      if (copy->PeekPacketTag(proTag)) {
        // cout<<"tagggggggggggggggggggggggggg--"<<proTag.GetHop()<<endl;
          if (proTag.GetHop() > 0) {
...
bool
ndnLcd::TrySendOutInterest (Ptr<Face> inFace,
                                        Ptr<Face> outFace,
                                        Ptr<const Interest> interest,
                                        Ptr<pit::Entry> pitEntry)
{
...
bool
ndnLcd::CanSendOutInterest (Ptr<Face> inFace,
                                        Ptr<Face> outFace,
                                        Ptr<const Interest> interest,
                                        Ptr<pit::Entry> pitEntry)
{
...
bool
ndnLcd::DoPropagateInterest (Ptr<Face> inFace,
                       Ptr<const Interest> interest,
                       Ptr<pit::Entry> pitEntry)
{
